...
---@param name string
---@return easydap.usercmd.SubcommandDef?
...
---@alias easydap.usercmd.subcommand_fn fun(cmd:string,rest:string[],arg_lead:string):string[]

---@alias easydap.usercmd.run_fn
---| fun(cmd:string,args:string[],opts:vim.api.keyset.create_user_command.command_args)


---@param subcommand_fn easydap.usercmd.subcommand_fn
...
local function _dispatch(cmd, run_fn, opts)
    ...
    if not ok then
        vim.notify(
            "[easydap.nvim] " .. cmd .. " command error\n" .. tostring(err),
...
---@param cmd string
---@param run_fn easydap.usercmd.run_fn
---@param opts {desc:string?,subcommand_fn:easydap.usercmd.subcommand_fn?}?
