...
-- TCP (writing DAP messages to its stdin is ignored). `_delve_setup` spawns it,
-- parses that line, and points the connection at the reported host/port.
-- `program` defaults to the current directory (debug the package at cwd). Field set
-- follows vscode-go's launch.json attributes for dlv-dap mode
...
---@param ctx      easydap.AdapterSetupCtx
---@param callback fun(err?: string, state?: any)
local function _delve_setup(config, ctx, callback)
    local term = require("easydap.tk.term")
    ...
    handle = term.spawn(cmd, {
        bufname   = _unique_buf_name("easydap://" .. (config.name or config.adapter or "debug") .. "/dlv-dap"),
        cwd       = config.cwd or vim.fn.getcwd(),
        env       = config.env,
