...
---with newlines), or nil when not in visual mode. Reaches the selection only
---while still in visual mode — e.g. an `x`-mode `<Cmd>Debug inspect<CR>` mapping,
...
local function _visual_selection()
    ...
    local region = vim.fn.getregion(p1, p2, { type = mode })
