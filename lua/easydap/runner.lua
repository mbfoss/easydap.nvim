...
---tokens. It finds the adapter's `target`-placeholder launch configuration (see
---`schema.target_configuration`) and fills it with `program`/`args`, then runs.
---Always a `launch` — adapters without such a configuration (attach-only ones) are
---rejected.
...
function M.run_target(adapter, program, program_args)
    ...
    local configuration_name = schema.target_configuration(adapter)
    if not configuration_name then
        _err("run_target: adapter " .. adapter .. " has no launch target configuration (try new_run_file)")
        return
    end
    ...
    if program_args and #program_args > 0 then
        if vim.tbl_contains(schema.configuration_placeholders(adapter, configuration_name), "args") then
    ...
    local params, _, err = schema.fill_configuration(adapter, configuration_name, values)
    if not params then
